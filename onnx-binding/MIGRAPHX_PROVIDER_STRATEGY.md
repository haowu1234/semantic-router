...
need graph preparation before ORT MIGraphX EP claims them. Intent and jailbreak
need the exported `com.microsoft::SkipLayerNormalization` node rewritten; under
...
and under ORT 1.25 with a MIGraphX 2.16 source build. Feedback SDPA did not
...
| --- | --- | --- | --- | ---: | --- |
| intent `model_sdpa_migraphx.onnx` | `e2e-domain` 261 rows | MIGraphX 2.15/2.16-source -> CPU | label match `1.0`; max logit drift `0.208907` on 2.15 and `0.228647` on 2.16-source | `2.728/2.760 ms` on 2.15 | generated by rewriting `model_sdpa_fp16.onnx`; 2.16-source ownership verified with ORT profiling |
| jailbreak `model_sdpa_migraphx.onnx` | `jailbreak-mixed` 100 rows | MIGraphX 2.15/2.16-source -> CPU | label match `1.0`; max logit drift `0.274858` on 2.15 and `0.209449` on 2.16-source | `2.714/3.101 ms` on 2.15 | generated by rewriting `model_sdpa_fp16.onnx`; 2.16-source ownership verified with ORT profiling |
...
- MIGraphX 2.15 owns rewritten sequence SDPA graphs but rejects public SDPA
  ...
  empty beta input. ORT 1.25 with a MIGraphX 2.16 source build additionally
  leaves intent and jailbreak CPU-owned until contrib Gelu, Split-18
  `num_outputs`, unused opset imports, and softmax NaN guards are rewritten.
  Feedback passed with the public SDPA artifact. Factcheck ran but did not meet
  the no-regression label-match gate on the 2.15 matrix.
- Sequence SDPA quality eval must use `--batch-size 1`. The router classifier
  path is single-request oriented, and the exported SDPA attention graph can hit
  ORT shape-reuse errors when evaluated with larger batches.
