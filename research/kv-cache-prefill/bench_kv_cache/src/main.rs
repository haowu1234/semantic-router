...
struct CachedLayer {
    ...
    v_proj: Linear,
    o_proj: Linear,
    kv_cache: candle_nn::kv_cache::SimpleKvCache,
}
...
impl CachedLayer {
    fn new(vb: VarBuilder) -> candle_core::Result<Self> {
        ...
        let kv_cache = candle_nn::kv_cache::SimpleKvCache::new();
    ...
    fn clear_cache(&mut self) {
        self.kv_cache = candle_nn::kv_cache::SimpleKvCache::new();
    }
}
...
impl MockModel {
    fn new(device: &Device) -> candle_core::Result<Self> {
        let vb = VarBuilder::randn(0.0, 0.02, device);
    ...
    fn forward(&mut self, token_ids: &[u32], offset: usize) -> candle_core::Result<Tensor> {
    ...
    fn kv_cache_snapshot(&self) -> Vec<candle_nn::kv_cache::SimpleKvCache> {
        self.layers.iter().map(|l| l.kv_cache.clone()).collect()
    }

    fn kv_cache_restore(&mut self, caches: &[candle_nn::kv_cache::SimpleKvCache]) {
...
fn main() -> candle_core::Result<()> {
    ...
    println!("Initializing mock model (random weights, CPU F32)...");
    ...
    println!("  clear → prefill(500) → snapshot → [restore → fwd(cat)] × N");
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");

    let mut with_times = Vec::new();
    for trial in 0..NUM_TRIALS {
        model.clear_kv_cache();
        let t_pre = measure_ms(|| { model.forward(&prefix_tokens, 0).unwrap(); });
        ...
        for ct in &cat_tokens {
            ...
            t_fwd_total += measure_ms(|| { model.forward(ct, PREFIX_TOKENS).unwrap(); });
        }
        let total = t_pre + t_snap + t_restore_total + t_fwd_total;
        with_times.push(total);
        println!("  Trial {}: prefill={:.1}ms snap={:.1}ms restores={:.1}ms({}×{:.2}ms) fwds={:.1}ms({}×{:.1}ms) TOTAL={:.1}ms",
    ...
    println!("  [clear → prefill(500) → fwd(cat)] × N");
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    ...
    for trial in 0..NUM_TRIALS {
        ...
        for ct in &cat_tokens {
            model.clear_kv_cache();
            t_pre_total += measure_ms(|| { model.forward(&prefix_tokens, 0).unwrap(); });
            t_fwd_total += measure_ms(|| { model.forward(ct, PREFIX_TOKENS).unwrap(); });
        }
        let total = t_pre_total + t_fwd_total;
        without_times.push(total);
        println!("  Trial {}: prefills={:.1}ms({}×{:.1}ms) fwds={:.1}ms({}×{:.1}ms) TOTAL={:.1}ms",
    ...
    println!("  Time saved:               {:.1}ms per classification", avg_without - avg_with);
    println!();
    ...
    for _ in 0..n_req {
        model.clear_kv_cache();
        guard_current += measure_ms(|| { model.forward(&prefix_tokens, 0).unwrap(); });
    }

    model.clear_kv_cache();
    let init_pre = measure_ms(|| { model.forward(&prefix_tokens, 0).unwrap(); });
    let snap = model.kv_cache_snapshot();
